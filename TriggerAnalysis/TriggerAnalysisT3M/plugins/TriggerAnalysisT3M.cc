...
private:
    ...
    edm::EDGetTokenT<edm::RangeMap<GEMDetId,edm::OwnVector<GEMRecHit,edm::ClonePolicy<GEMRecHit> >,edm::ClonePolicy<GEMRecHit> > > gemRecHitToken_;
    edm::EDGetTokenT<MuonDigiCollection<GEMDetId,GEMPadDigiCluster> > gemTPToken_;
  ...
  //    TH1F *hEventsAfterGoodCand;
   edm::InputTag algInputTag_;
    ...
    //MuonID
    std::vector<double>  Muon_isGlobal,  Muon_isTracker, Muon_isGEM, Muon_isSoft,  Muon_isLoose, Muon_isTight,  Muon_isPF,  Muon_isRPCMuon,  Muon_isStandAloneMuon,  Muon_isTrackerMuon,  Muon_isCaloMuon,  Muon_isQualityValid,  Muon_isTimeValid,  Muon_isIsolationValid,  Muon_numberOfMatchedStations,  Muon_numberOfMatches, Muon_SoftMVAVal, Muon_LowPtMVA_Val;
    ...
    std::vector<double>  GEMRecHit_region, GEMRecHit_ring, GEMRecHit_station, GEMRecHit_layer, GEMRecHit_chamber, GEMRecHit_roll, GEM_TP;
    ...
    TriggerAnalysisT3M::TriggerAnalysisT3M(const edm::ParameterSet& iConfig){
        ...
        gemRecHitToken_ = consumes<edm::RangeMap<GEMDetId,edm::OwnVector<GEMRecHit,edm::ClonePolicy<GEMRecHit> >,edm::ClonePolicy<GEMRecHit> >> (edm::InputTag("gemRecHits", "", "RECO" ));
        gemTPToken_ = consumes<MuonDigiCollection<GEMDetId,GEMPadDigiCluster> > (edm::InputTag("simMuonGEMPadDigiClusters","","HLT" ));
    }
    ...
    //    edm::Handle<GEMRecHitCollection> rechit_collection;
        edm::Handle<edm::RangeMap<GEMDetId,edm::OwnVector<GEMRecHit,edm::ClonePolicy<GEMRecHit> >,edm::ClonePolicy<GEMRecHit> > > rechit_collection;
        iEvent.getByToken(gemRecHitToken_, rechit_collection);
        if (not rechit_collection.isValid()) {
            std::cout << "GEMRecHitCollection is invalid" << std::endl;
            return;
        }
        
        edm::Handle<MuonDigiCollection<GEMDetId,GEMPadDigiCluster> > gemTP_collection;
        iEvent.getByToken(gemTPToken_, gemTP_collection);
        if (not gemTP_collection.isValid()) {
            std::cout << "gemTPCollection is invalid" << std::endl;
            return;
        }
            ...
            if (rechit_collection.isValid()){
                ...
                for(; recHitMu != recHitMuEnd; ++recHitMu){
                    ...
                    if(detId.det() == DetId::Muon && detId.subdetId() == MuonSubdetId::GEM){
                        count_RecHit++;
                        cout << "GEM recHit n " << count_RecHit <<  endl;
                        const GEMRecHit *gemRecHit = dynamic_cast<const GEMRecHit *>(*recHitMu);
                        if (gemRecHit == nullptr){
                            std::cout << " gemRecHit dynamic cast failure" << std::endl;
                            continue;
                        }
                        GEMRecHit_region.push_back(gemRecHit->gemId().region());
                        GEMRecHit_ring.push_back(gemRecHit->gemId().ring());
                        GEMRecHit_station.push_back(gemRecHit->gemId().station());
                        GEMRecHit_layer.push_back(gemRecHit->gemId().layer());
                        GEMRecHit_chamber.push_back(gemRecHit->gemId().chamber());
                        cout << "Chamber " << gemRecHit->gemId().chamber() << endl;
                        GEMRecHit_roll.push_back(gemRecHit->gemId().roll());
                        cout << "Roll " << gemRecHit->gemId().roll() << endl;
    
                        // Check presence of GEM trigger primitive
                        int count_TP = 0;
                        if (gemTP_collection.isValid()){
                            auto GEMPadDigiCluster_TP = gemTP_collection->get(gemRecHit->gemId());
//                                cout << "### Detector " << gemRecHit->gemId() << endl;
                            auto GEMPadDigiCluster_Init = std::get<0>(GEMPadDigiCluster_TP);
                            auto GEMPadDigiCluster_End = std::get<1>(GEMPadDigiCluster_TP);
                            for(auto GEMPadDigiCluster_It = GEMPadDigiCluster_Init; GEMPadDigiCluster_It != GEMPadDigiCluster_End; GEMPadDigiCluster_It++){
                                count_TP++; GEM_TP_present = true;
                            }
                        }
//                        GEM_TP.push_back(count_TP);
                    }
                    else{
                        GEMRecHit_region.push_back(-999);
                        GEMRecHit_ring.push_back(-999);
                        GEMRecHit_station.push_back(-999);
                        GEMRecHit_layer.push_back(-999);
                        GEMRecHit_chamber.push_back(-999);
                        GEMRecHit_roll.push_back(-999);
//                        GEM_TP.push_back(-999);
                    }
                } // end loop RecHit
                if(GEM_TP_present) GEM_TP.push_back(1);
                else GEM_TP.push_back(0);
            }
            else{
                GEMRecHit_region.push_back(-999);
                GEMRecHit_ring.push_back(-999);
    ...
    GEMRecHit_region.clear();
    GEMRecHit_ring.clear();
    GEMRecHit_station.clear();
    GEMRecHit_layer.clear();
    GEMRecHit_chamber.clear();
    GEMRecHit_roll.clear();
    GEM_TP.clear();
    ...
    tree_->Branch("GEMRecHit_region", &GEMRecHit_region);
    tree_->Branch("GEMRecHit_ring", &GEMRecHit_ring);
    tree_->Branch("GEMRecHit_station", &GEMRecHit_station);
    tree_->Branch("GEMRecHit_layer", &GEMRecHit_layer);
    tree_->Branch("GEMRecHit_chamber", &GEMRecHit_chamber);
    tree_->Branch("GEMRecHit_roll", &GEMRecHit_roll);
    tree_->Branch("GEM_TP", &GEM_TP);
